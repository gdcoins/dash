...
const char * const BITCOIN_CONF_FILENAME = "gdcoin.conf";
const char * const BITCOIN_PID_FILENAME = "gdcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gdcoin" is a composite category enabling all Gdcoin-related debug output
            if(ptrCategory->count(string("gdcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GdcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GdcoinCore
    // Mac: ~/Library/Application Support/GdcoinCore
    // Unix: ~/.gdcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GdcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GdcoinCore";
#else
    // Unix
    return pathRet / ".gdcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gdcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gdcoin.conf
